...
import { getPureGoldPrice } from "../services/goldPriceService";
import { ProductEntry, PricedProductEntry } from "../types";
...
const toPricedProductEntry = (product: ProductEntry, currentGoldPrice: number): PricedProductEntry => {
  const pricedProduct: PricedProductEntry = {
    id: uuidv4(),
    ...product,
    price: Number(((product.popularityScore + 1) * product.weight * currentGoldPrice).toFixed(2))
  };
  return pricedProduct;
};

export const generatePricedProducts = async (): Promise<PricedProductEntry[]> => {
  const currentGoldPrice = await getPureGoldPrice();
  if (!currentGoldPrice) {
    throw new Error('Could not retrieve current gold price');
  }

  const pricedProducts = products.map((product) =>
    toPricedProductEntry(product, currentGoldPrice)
  );
